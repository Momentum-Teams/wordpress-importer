...
const settings_defaults = {
  ...
  import_assets: {
    ...
    types: ['png', 'jpg', 'jpeg', 'gif', 'png', 'svg', 'pdf', 'webp'],
...
export default class Wp2Storyblok {
  ...
  constructor(endpoint, settings = {}) {
    ...
    this.settings.content_types.sort((a, b) => {
      if (
        this.settings.content_types.find((ct) =>
          ct.taxonomies?.find((t) => t.name === a.name)
        )
      ) {
        return -1;
      }
      if (
        this.settings.content_types.find((ct) =>
          ct.taxonomies?.find((t) => t.name === b.name)
        )
      ) {
        return 1;
      }
  ...
  async prepareStories() {
    ...
    for (let i = 0; i < this.settings.content_types.length; i++) {
      ...
      for (
        let j = 0;
        j < this.wp.content_types[content_type.name].length;
        j++
      ) {
        ...
        if (content_type.folder) {
          ...
          wp_entry.link = wp_entry.link.replace(
            entry_url.origin,
            `${entry_url.origin}/${content_type.folder
              .replace(/^\//, '')
              .replace(/\/$/, '')}`
          );
        }
        ...
        const component_name =
          content_type.new_content_type || content_type.name;
        let sb_entry = {
        ...
        if (
          sb_links.stories.find((l) => compareSlugs(l.slug, sb_entry._wp_link))
        ) {
          continue;
        }
        ...
        try {
          ...
          if (
            !this.folders_to_migrate.find(
              (f) => f.path === sb_entry['_wp_folder']
            ) &&
            !sb_links.folders.find((f) =>
              compareSlugs(f.slug, sb_entry['_wp_folder'])
            )
          ) {
            const folder_slug =
              sb_entry['_wp_folder'].split('/')[
                sb_entry['_wp_folder'].split('/').length - 2
              ];
  ...
  async importAssets() {
    const assets_regex = new RegExp(
      `(\\"((http)?s?:?(\\/?\\/[^"]*.(${this.settings.import_assets.types.join(
        '|'
      )})))(\\\\)?")`,
      'g'
    );
    let regex_results = [];
    while (
      (regex_results = assets_regex.exec(
        JSON.stringify(this.stories_to_migrate)
      ))
    ) {
      if (
        !this.assets_to_migrate.find(
          (asset) => asset.original_url == regex_results[2]
        )
      ) {
        const full_url =
          regex_results[2][0] == '/'
            ? `${this.settings.domain}${regex_results[2]}`
            : regex_results[2];
        this.assets_to_migrate.push({
          original_url: regex_results[2],
          full_url,
        });
      }
    }
    this.migrated_assets = await this.storyblok.uploadAssets(
      this.assets_to_migrate.map((a) => a.full_url)
    );
  }
  ...
  replaceAssetsUrls() {
    this.migrated_assets.forEach((asset) => {
      try {
        ...
        this.stories_to_migrate = JSON.parse(
          JSON.stringify(this.stories_to_migrate).replace(reg, asset.new_url)
        );
      } catch (err) {
  ...
  async importStories() {
    ...
    const stories = await this.storyblok.createStories(
      this.stories_to_migrate,
      this.taxonomiesData
    );
    if (this.stories_to_migrate.length) {
  ...
  async importFolders() {
    ...
    for (let i = 0; i < this.folders_to_migrate.length; i++) {
      const starte_page = this.stories_to_migrate.find(
        (s) => s._wp_link === this.folders_to_migrate[i].path
      );
      if (starte_page) {
    ...
    for (let j = 0; j < this.folders_to_migrate.length; j++) {
      if (this.folders_to_migrate[j].path.split('/').length > 3) {
        this.folders_to_migrate[j].parent = `${this.folders_to_migrate[j].path
          .split('/')
          .slice(0, -2)
          .join('/')}/`;
        if (
          !this.folders_to_migrate.find(
            (f) => f.path === this.folders_to_migrate[j].parent
          ) &&
          !sb_links.folders.find((f) =>
            compareSlugs(f.slug, this.folders_to_migrate[j].parent)
          )
        ) {
          const folder_slug =
            this.folders_to_migrate[j].parent.split('/')[
              this.folders_to_migrate[j].parent.split('/').length - 2
            ];
          this.folders_to_migrate.push({
    ...
    this.folders_to_migrate = this.folders_to_migrate.filter(
      (f) => f.path !== '/'
    );

    // Create the folders
    this.created_folders = await this.storyblok.createFolders(
      this.folders_to_migrate
    );
    ...
    this.stories_to_migrate
      .filter((s) => s._wp_folder)
      .forEach((story) => {
        const folder = this.created_folders.find(
          (f) => f.path === story._wp_folder
        );
        if (folder) {
          ...
          const sb_folder = links.folders.find((l) =>
            compareSlugs(l.slug, story._wp_folder)
          );
          if (sb_folder) {
  ...
  async formatFieldForStoryblok(field_value, field, component_name) {
    ...
    if (typeof field === 'object' && field.component_field) {
      type = this.storyblok.getFieldType(
        field.component,
  ...
  async getGutenbergBlocks(blocks) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      let block_data = {};
      const block_mapping = this.settings.blocks_mapping?.find(
        (b) => b.name === block.blockName
      );
      if (block_mapping) {
        // In case there's a custom mapping, it'll be used
        block_data.component =
          block_mapping.new_block_name || block_mapping.name;
        const wp_block_data = await this.populateFields(
          block,
          block_data.component,
          block_mapping.schema_mapping
        );
  ...
  async populateFields(data, component_name, mapping, taxonomies) {
    ...
    for (const [source, target] of Object.entries(mapping)) {
      ...
      if (taxonomies) {
        unformatted_field_value = this.wp.filterTaxonomyValue(
          taxonomies,
          unformatted_field_value,
          source
        );
      }
      const field_value = await this.formatFieldForStoryblok(
        unformatted_field_value,
        target,
        component_name
      );
  ...
  get taxonomiesData() {
    return this.settings.content_types
      ...
      .map((content_type) =>
        content_type.taxonomies.map((taxonomy) => {
          const taxonomy_content_type = this.settings.content_types.find(
            (ct) => ct.name === taxonomy.name
          );
          const field_name =
            typeof content_type.schema_mapping[taxonomy.field] === 'string'
              ? content_type.schema_mapping[taxonomy.field]
              : content_type.schema_mapping[taxonomy.field].field;
          return {
            content_type: content_type.new_content_type,
            field: field_name.replace('content.', ''),
            taxonomy: taxonomy_content_type.new_content_type,
            type: taxonomy.type || 'relationship',
          };
        })
      )
      .flat();
